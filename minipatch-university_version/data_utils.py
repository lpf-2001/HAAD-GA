...
base_path = '/home/xuke/xrt/datasets/'  # D:/XRT/HNU/Dataset/网络数据包流量/WF-Datasets/
...
def load_awf_dataset(input_size=5000, num_classes=103, formatting=True, file_name='awf2_remain_small200and100.npz'
                     ,valid_size=0.05, test_size=0.05):
    # Point to the directory storing data
    dataset_dir = base_path+'AWF/'

    # Load data
    datafile = dataset_dir + file_name
    with np.load(datafile, allow_pickle=True) as npzdata:
        data = npzdata['data']
        labels = npzdata['labels']
    ...
    # Split data to fixed parts
    X_train, y_train, X_valid, y_valid, X_test, y_test = train_test_valid_split(data, y, valid_size=valid_size, test_size=test_size)
    with open(dataset_dir + 'tor_%dw_2500tr_test.npz' % num_classes, 'wb') as handle:
        pickle.dump({'X_test': X_test, 'y_test': y_test}, handle)
...
def load_awf_websites(num_classes=100):
    """
    Load website labels of awf's dataset.
    """
    # Point to the directory storing data
    dataset_dir = base_path+'AWF/'

    # Load data
    datafile = dataset_dir + 'awf2_small200.npz'
    with np.load(datafile, allow_pickle=True) as npzdata:
        labels = npzdata['labels']
