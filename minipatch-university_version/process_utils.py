...
K.set_image_data_format('channels_last')  # todo: channels_first is error setting!
from tensorflow.keras.models import load_model

from data_utils import load_sirinam_dataset
from data_utils import load_rimmer_dataset, load_rimmer_websites, load_awf_websites, load_awf_dataset
...
def load_data(dataset, input_size, num_classes, data, verbose=1):
    ...
    elif dataset.lower().startswith('awf'):
        names = load_awf_websites(num_classes)
        if data == 'test':
            # awf2_small200 is test set file
            traces, labels, _, _, _, _ = load_awf_dataset(input_size, num_classes, file_name='awf2_small200.npz'
                                                          , valid_size=0.01, test_size=0.01)
        elif data == 'valid':
            _, _, traces, labels, _, _ = load_awf_dataset(input_size, num_classes)
        else:
            raise Exception('Sirinam supports only test and valid data')
